...
app.post('/approve', function (req, res) {
	 ...
	 * reqid is sent as a hidden parameter to the HTML page, need to parse this out.
	 * It was used to store the request itself, to prevent a cross-site forgery attack
	 */
	...
	if (req.body.approve) {
		if (query.response_type == 'code') {
			var code = randomstring.generate({ length: 16, readable: true });
...
app.post("/token", function (req, res) {
	...
	if (req.body.client_id) {
		if (clientId) {
			res.status(401).json({ error: 'invalid_client' }).end();
			return;
		}
		var clientId = req.body.client_id;
		var clientSecret = req.body.client_secret;
	}
	// has the client been registered and if so, is its secret correct?
	...
	if (req.body.grant_type == 'authorization_code') {
		...
		if (code) {
			...
			if (code.request.client_id == clientId) {
				...
				res.status(400).json({ error: 'invalid_grant' }).end();
				return;
		...
		res.status(400).json({ error: 'unsupported_grant_type' }).end();
		return;
	}
});
